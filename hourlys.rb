...
class Elmeter
	...
	def initialize(filename, alias_name)
		...
		# считываем третью строку файла где записаны нормер счетчика и его владелец
		3.times {inputline=input.gets.strip}

		# определяем позицию наименвоания счетчика по последнему вхождению в строку подстроки с двумя цифрами и точкой после них
